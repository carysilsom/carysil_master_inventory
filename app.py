...
# Calculations block
        ...
        for item in items:
            ...
            WHERE
            UPPER(TRIM(product_name))=UPPER(TRIM(%s))

            AND

            UPPER(TRIM(product_size))=UPPER(TRIM(%s))

            AND

            UPPER(TRIM(colour))=UPPER(TRIM(%s))
...
"CARYSIL - SOM ASSOCIATES STOCK MANAGEMENT INVENTORY",
...
"OFFICIAL INVENTORY SYSTEM DISTRIBUTOR LOG",
...
f"<b>Customer Name:</b> {party_name}",
...
colWidths=[25,210,65,70,35,70,55,80]
